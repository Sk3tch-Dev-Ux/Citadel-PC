class Citadel_Desk: BaseLaptop
{
	...
	override void OnWorkStart()
	{
		...
		GetCitadelPcLogger().LogInfo("Citadel_Desk: Starting up...");
	...
	void FinishStartup()
	{
		...
		GetCitadelPcLogger().LogInfo("Citadel_Desk: Ready");
	}
	...
	override void OnWorkStop()
	{
		...
		GetCitadelPcLogger().LogInfo("Citadel_Desk: Shutting down...");
	...
	void FinishShutdown()
	{
		...
		GetCitadelPcLogger().LogInfo("Citadel_Desk: Powered off");
	}
	...
	void StartMining()
	{
		...
		CitadelPcSettings settings = GetCitadelPcModule().GetSettings();
		if (!settings)
		{
			GetCitadelPcLogger().LogError("Citadel_Desk: StartMining — settings unavailable");
			return;
		}

		bool anyStarted = false;

		foreach (string coinName, CitadelCoinSettings coinCfg : settings.coinSettings)
		{
			foreach (string slotName : g_CitadelPcGraphicCardSlots)
			{
				...
				if (coinCfg.CanCardMine(gpu.GetType()))
				{
					...
					anyStarted = true;
					GetCitadelPcLogger().LogInfo("Citadel_Desk: Mining " + coinName + " with " + gpu.GetType() + " [" + slotName + "]");
					break;
				}
			}
		}
		...
		if (anyStarted)
		{
			...
			GetCitadelPcLogger().LogInfo("Citadel_Desk: Mining started");
		}
		else
		{
			GetCitadelPcLogger().LogWarning("Citadel_Desk: StartMining — no compatible GPUs found for any configured coin");
		}
	}
	...
	void StopMining(string coinName)
	{
		...
		GetCitadelPcLogger().LogInfo("Citadel_Desk: Stopped mining " + coinName);
		...
		if (!IsAnyMiningActive())
		{
			...
			GetCitadelPcLogger().LogInfo("Citadel_Desk: All mining stopped");
		}
	}
	...
	void MiningTick()
	{
		...
		foreach (string coinName, bool active : m_CoinMiningStatus)
		{
			...
			CitadelCoinSettings coinCfg = settings.GetCoinSettings(coinName);
			if (!coinCfg)
				continue;
			...
			foreach (string slotName : g_CitadelPcGraphicCardSlots)
			{
				...
				GetCitadelPcLogger().LogDebug("Citadel_Desk: +" + yield + " " + coinName + " | balance: " + (balance + yield) + " | GPU: " + gpuType);
				...
				if (gpu.IsRuined())
				{
					GetCitadelPcLogger().LogWarning("Citadel_Desk: GPU " + gpuType + " ruined — stopping " + coinName);
					StopMining(coinName);
					break;
				}
			}
	...
	void WithdrawCoins(PlayerBase player)
	{
		...
		foreach (string coinName, float amount : m_MinedCoins)
		{
			...
			if (!item)
			{
				...
				GetCitadelPcLogger().LogWarning("Citadel_Desk: No inventory space for " + coinName + " — dropped on ground");
			}
			...
			if (item)
			{
				...
				GetCitadelPcLogger().LogInfo("Citadel_Desk: Withdrew " + amount + " " + coinName);
			}
		}
	}
