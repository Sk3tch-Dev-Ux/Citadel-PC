class ActionBuildCitadelDeskCB : ActionContinuousBaseCB
{
...
class ActionBuildCitadelDesk : ActionContinuousBase
{
    void ActionBuildCitadelDesk()
    {
        m_CallbackClass = ActionBuildCitadelDeskCB; // Using the vanilla callback class
    ...
    override void OnFinishProgressServer(ActionData action_data)
    {
        ...
        Citadel_Desk_Build hologramBuild;
        ...
        if (CastTo(hologramBuild, target_object))
        {
            ...
            EntityAI hologram = EntityAI.Cast(GetGame().CreateObjectEx("Citadel_Desk", position, ECE_PLACE_ON_SURFACE));
